    ...
    items: [
      {
        ...
        bullets: [
          ...
          "Built remote over-the-air bridge updates with automatic rollback: an admin picks any target version from the dashboard, the desktop (Tauri) bridge applies a signature-verified installer while streaming live download progress back over MQTT, and the Raspberry Pi bridge does an atomic install-directory swap gated by a post-restart health check that auto-reverts to the prior version if the new build fails to come up. Any version is a valid target, so remotely rolling a fleet back after a bad release is a one-click action.",
          "Built a remote-view feature that mirrors a bridge's live UI into any browser over MQTT: a short-lived org-scoped session token connects to AWS IoT, the bridge streams chunked state snapshots every few seconds, and the page renders the exact same shared React components as the native desktop app — so support can watch printers, the queue, logs, and history in real time and even invoke commands remotely, identical to standing in front of the device.",
